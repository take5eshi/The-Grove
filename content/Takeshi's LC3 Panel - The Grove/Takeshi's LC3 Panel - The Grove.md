Welcome! In case you couldn't tell by the name, I'm Takeshi, a 9th grader at the time of writing this. This is the grove, built using [Quartz](https://quartz.jzhao.xyz/), an Obsidian Publish clone for brokie's like me. The idea to put my portfolio on here originated as a temporary solution to the problem/mess that is my filthy google drive, but as I have learned and failed, and tried again, and failed again, and hopefully succeeded, I have grown fond of Quartz and the Obsidian UI. Needless to say, this won't be my last time using both. 

# Where to Start
I've designed this portfolio to be much more user friendly and easier to understand than its [previous google drive format](https://drive.google.com/drive/folders/1EzHlWXpK8IMHVVfqIDLtPkL0XpIY30Vv?usp=sharing). There is a specified route which allows users to simply start at a point and work their way through the portfolio, without getting lost or wondering what exactly they are looking at. So, let's get you started.
 - [[Takeshi's LC3 Panel - The Grove]]
	- [[Advisory]]
		 - [[Current Events]]
		 - [[Health]]
		 - [[Japanese Seminar]]
		 - [[Literary Journalism]]
		 - [[WITS]]
	- [[LTI]]
		- [[Interviews]]
		- Shadow Days
		- [[LTI Tracker]]
		- Scripts, Questions, and more!
	- Projects
		- Obsidian Coding
		- Piano
		- Running a 10k
		- Robotics
 - [[content/Takeshi's LC3 Panel - The Grove/Index|Index]]
 - [[content/Takeshi's LC3 Panel - The Grove/README|README]]
 - 

## Odd Commodities
As confusing as this may seem, it is nowhere as intense as some of the more major vaults on the internet, namely Nicole van der Hoeven's "Fork my Brain" vault that has the capability of crashing my computer.